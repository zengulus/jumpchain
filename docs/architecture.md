...
* Narration constructs tracker, current scene, NPC objective/epistemic, lore, memory, and exchange candidates. Worldbook chunks carry a book-qualified logical-source group key; the lore pool's `groupCount` cap (ceil of half the lore depth) bounds chunks admitted per WorldEntry so one long source cannot monopolize the small lore budget — multiple chunks of one source remain admissible when competitors are weaker, ordering is untouched, and skipped chunks are recorded as `pool-group` omissions. Mechanics and chat retain token caps; loreDepth and memoryDepth stay count caps on admitted records. A too-large optional record does not consume a count slot, so a smaller subsequent candidate can fit. All search matches reach admission, removing the former global top-30 starvation behavior. Cross-class optional candidates now compete by salience and relevance rather than incidental call order; current lexical and retrieval signals retain their existing scales.
