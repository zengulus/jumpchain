...
* One semantic canonical projection (`retrieval.ts::searchableText`: title, entities/aliases, tags, text, each once) feeds embedding index construction, reranking inputs, and the index fingerprint. BM25 keeps an explicit title boost in the lexical scorer instead of duplicating the title in the semantic representation. The fingerprint covers exactly the embedding inputs (record ID plus projection), so unrelated metadata edits (Jump scope, authority, chronology, provenance) leave stored vectors valid. Narration-time lore queries are built by one pure helper (`narrationLoreQuery`: action plus the scene's location and a bounded amount of active thread context), shared by generation retrieval and mechanics-pool scoring; these are ranking signals only — never activation triggers, hard filters, or SillyTavern key emulation.
* Narration constructs tracker, current scene, NPC objective/epistemic, lore, memory, and exchange candidates. Worldbook chunks carry a book-qualified logical-source group key; the lore pool applies soft diversity (`groupMarginal` with an explicit `rankPenalty`, currently 2): pool candidates are ranked by raw relevance and each further chunk of one entry is demoted two raw-rank positions per same-source candidate ranked ahead of it — calibrated for 4–8-slot lore budgets — so fresh relevant sources usually precede redundant repeats while a genuinely strong repeat still beats a weak fresh source and unused lore capacity can fill from one source. Only admission order changes — raw retrieval ranking is untouched — and hard per-source quotas remain available as `groupCount`. Mechanics and chat retain token caps; loreDepth and memoryDepth stay count caps on admitted records. A too-large optional record does not consume a count slot, so a smaller subsequent candidate can fit. All search matches reach admission, removing the former global top-30 starvation behavior. Cross-class optional candidates now compete by salience and relevance rather than incidental call order; current lexical and retrieval signals retain their existing scales.
