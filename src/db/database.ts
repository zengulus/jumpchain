  ...
  constructor() {
    ...
    this.version(3)
      .stores({
        ...
        companions: '&id, chainId, branchId, parentJumperId, updatedAt',
        jumps: '&id, chainId, branchId, orderIndex, status, updatedAt',
        jumpDocs: '&id, chainId, branchId, updatedAt',
        participations: '&id, chainId, branchId, jumpId, jumperId, [jumpId+jumperId], updatedAt',
    ...
    this.version(4)
      ...
      .upgrade(async (tx) => {
        ...
        for (const table of participationTables) {
          ...
          await table.bulkPut(records.map((record) => ({
            ...record,
            purchases: normalizeParticipationSelections(record.purchases, 'purchase'),
            drawbacks: normalizeParticipationSelections(record.drawbacks, 'drawback'),
            retainedDrawbacks: normalizeParticipationSelections(record.retainedDrawbacks, 'retained-drawback'),
            currencyExchanges: record.currencyExchanges.map((exchange) => normalizeCurrencyExchange(exchange)),
          })));
        }
      });
  }
