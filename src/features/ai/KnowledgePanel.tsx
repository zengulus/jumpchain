...
export function KnowledgePanel({campaign,bundle,onSave,onOperation}:{campaign:Campaign;bundle:NativeChainBundle;onSave:(books:Worldbook[])=>Promise<void>;onOperation:(operation:string,body?:unknown)=>Promise<unknown>}) {
  ...
  async function importFile(file:File) {
    ...
    if(/\.pdf$/i.test(file.name)) {
      ...
      await onSave([...campaign.worldbooks,next]);setSelected(next.id);setNotice('Worldbook imported. Rebuild embeddings if configured.');
    }
  }
  return <section className="ai-panel stack"><h2>World knowledge & source documents</h2><p>Sources are portable campaign data. Search indexes can be rebuilt or deleted independently.</p>
    <fieldset disabled={busy}><legend>Import sources</legend><label>Setting<input value={setting} onChange={e=>setSetting(e.target.value)}/></label><label>Import text, Markdown, worldbook JSON, or PDF<input type="file" accept=".txt,.md,.json,.pdf" onChange={e=>{const f=e.target.files?.[0];if(f)void run(()=>importFile(f));e.target.value='';}}/></label>
