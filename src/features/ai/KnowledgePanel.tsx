...
export function KnowledgePanel({campaign,bundle,onSave,onOperation}:{campaign:Campaign;bundle:NativeChainBundle;onSave:(books:Worldbook[])=>Promise<void>;onOperation:(operation:string,body?:unknown)=>Promise<unknown>}) {
  ...
  // Worldbooks are owned by the Jump the campaign is currently in; imports/creations default to
  // this scope. The label below surfaces ownership to the player.
  const currentJumpId=campaign.state.scene.stamp.jumpId;
  const branchJumps=bundle.jumps.filter(j=>j.branchId===campaign.branchId);
  const jumpTitle=(jumpId:string)=>branchJumps.find(j=>j.id===jumpId)?.title;
  const jumpLabel=(jumpId:string)=>jumpTitle(jumpId) ?? `Unknown jump (${jumpId.slice(0,8)}…)`;
  ...
  async function importFile(file:File) {
    ...
    if(/\.pdf$/i.test(file.name)) {
      ...
      const next=importWorldbook(await file.text(),file.name,crypto.randomUUID(),{currentJumpId,setting});
