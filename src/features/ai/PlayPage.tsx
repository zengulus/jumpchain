  ...
  return <>
    ...
    {tab==='setup'?<SetupPanel onConnected={()=>void work(async()=>{await refreshList();})}/>:<>
      ...
      {campaign&&<>
        ...
        {tab==='knowledge'&&<Suspense fallback={<p>Loading knowledge tools…</p>}><KnowledgePanel campaign={campaign} bundle={bundle} onSave={async worldbooks=>{await operation('worldbooks',{worldbooks});}} onOperation={(op,body)=>operation(op,body as Record<string,unknown>)}/></Suspense>}
