...
import type { Campaign, CompiledContext, ProviderConfig } from './schema';
...
export function compileContext(bundle: NativeChainBundle, campaign: Campaign, action: string, provider: ProviderConfig, retrieved: Retrieved[] = [], diagnostics: string[] = []): CompiledContext {
  ...
  const add = (name: string, value: unknown, ids: string[] = [], required = false, allowance = Infinity) => {
    ...
    context.layers.push({name, content, sourceIds: ids, estimatedTokens: count}); context.estimatedTokens += count; return true;
  };
  // Reserve action and hard constraints before relevance-selected material.
  add('GM system rules', settings.gmPrompt, [], true);
  add('Campaign style and rules', { ...settings, gmPrompt: undefined }, [], true);
  for (const r of records.filter(r => r.required)) add(`Authoritative ${r.category}`, { id: r.id, owner: r.owner, authority: 'authoritative', value: r.record }, [r.id], true);
  add('Current jump', {id: campaign.state.scene.stamp.jumpId, title: bundle.jumps.find(j => j.id === campaign.state.scene.stamp.jumpId)?.title, documents: bundle.jumpDocs.filter(d => bundle.jumps.find(j => j.id === campaign.state.scene.stamp.jumpId)?.jumpDocIds.includes(d.id)).map(d => ({id:d.id,title:d.title,author:d.author,source:d.source,notes:d.notes}))}, [], true);
  add('Current scene facts', campaign.state.scene, [], true);
  add('Current user action', action, [], true);
  // NPC state is first-class. Never serialize beliefs as reality.
  for (const npc of campaign.state.npcs.filter(n => campaign.state.scene.npcIds.includes(n.id) || campaign.state.scene.presentCompanionIds.includes(n.companionId ?? '') || tokens(`${n.name} ${n.aliases.join(' ')}`).some(t => tokens(action).includes(t)))) add('NPC beliefs and knowledge (not objective reality)', npc, [npc.id], true);
  ...
  for (const {r} of optional) {
    ...
    add(`Authoritative ${r.category}`, {id: r.id, owner: r.owner, authority: r.category === 'player-note' ? 'player-established' : 'authoritative', value: r.record}, [r.id], false, mechanicsLeft);
    mechanicsLeft -= context.estimatedTokens-before;
  }
  for (const r of retrieved) add(r.record.sourceType === 'world' ? 'Retrieved world lore' : 'Retrieved campaign memories', { ...r.record, selectionReason: r.reason }, [r.record.id]);
  ...
  context.layers.push({name:'Recent conversation',content:stableStringify(history),sourceIds:[],estimatedTokens:settings.chatBudget-chatLeft});
